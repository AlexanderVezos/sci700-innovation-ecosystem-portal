...
function bubbleR(employees, type, viewportMin, nodeCount = 1) {
  const baseline = TYPE_BASELINE[type] ?? 12;
  ...
  const t =
    (Math.min(65, Math.sqrt(Math.max(employees || baseline, 1)) * 9) - 30) / 35;
...
function EcosystemMap() {
        ...
        if (sim && sim.nodes().length > 0) {
          ...
          if (brandNew.length > 0) {
            ...
            const newSimNodes = brandNew.map((s, i) => ({
              ...
              r: bubbleR(s.employees, s.type, vMin, total),
  ...
  useEffect(() => {
    ...
    for (const n of sim.nodes()) n.r = bubbleR(n.employees, n.type, vMin, total);
    sim.force(
      "collide",
      forceCollide((d) => d.r + 4)
        .strength(0.9)
