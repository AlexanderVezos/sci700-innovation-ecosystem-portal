...
function buildCalendarCells(year, month) {
  // +6 % 7 shifts Sunday from JS getDay() position 0 to position 6, giving a Monday-first grid.
  // Day 0 of month+1 is the last day of the current month.
...
function EventCard({ event }) {
  ...
  return (
    <div
      className={`bg-white rounded-2xl shadow-sm border p-6 flex gap-5 items-start w-full transition-opacity ${isPast ? "opacity-50 border-slate-100" : "border-slate-100"}`}
    >
      <DateBlock iso={event.date} />
      <div className="flex-1 min-w-0">
        <div className="flex items-start gap-2 flex-wrap">
          <span className="font-bold text-slate-800 text-lg leading-snug">
            {event.title}
          </span>
          <div className="flex items-center gap-1.5 flex-wrap mt-0.5">
            <span
              className={`text-xs font-medium px-2.5 py-0.5 rounded-full ${typeColour}`}
            >
              {event.type}
            </span>
            {isPast && (
              <span className="text-xs font-medium px-2.5 py-0.5 rounded-full bg-slate-100 text-slate-400">
                Past
              </span>
            )}
          </div>
                  ...
                  {dayEvents.length > 0 && (
                    <div className="flex gap-0.5 mt-1 flex-wrap justify-center px-1">
                      {dayEvents.slice(0, 3).map((e) => (
                        <span
                          key={e._id}
                          className={`w-1.5 h-1.5 rounded-full ${isSelected ? "bg-white/70" : (TYPE_DOT[e.type] ?? TYPE_DOT.Other)}`}
                        />
                      ))}
                      {dayEvents.length > 3 && (
                        <span
                          className={`text-[9px] font-bold leading-none ${isSelected ? "text-white/70" : "text-slate-400"}`}
                        >
                          +{dayEvents.length - 3}
                        </span>
                      )}
...
export default function EventsPanel() {
    ...
    fetch("/api/events")
      ...
      .then((data) => {
        if (seenIds.current !== null) {
          const brandNew = data
            .filter((e) => !seenIds.current.has(e._id))
            .map((e) => e._id);
  ...
  const visible = events
    .filter((e) => {
      ...
      if (filters.dateFrom && (e.date ?? "").split("T")[0] < filters.dateFrom)
        return false;
      if (filters.dateTo && (e.date ?? "").split("T")[0] > filters.dateTo)
        return false;
    ...
    .sort((a, b) => {
      ...
      if (field === "date") return mul * (aDate - bDate);
      if (field === "createdAt")
                      ...
                      {visible.map((event) => {
                        ...
                        return (
                          <motion.div
                            ...
                            layout
                            initial={
                              !reduceMotion ? { opacity: 0, y: 8 } : false
                            }
                            animate={{ opacity: 1, y: 0 }}
                            exit={
                              !reduceMotion ? { opacity: 0, y: -4 } : undefined
                            }
                            transition={{
