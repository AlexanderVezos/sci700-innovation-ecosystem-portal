...
function Navbar() {
    ...
    return (
    <nav className="flex items-center justify-between px-8 py-4 bg-white shadow-lg relative z-10">
        <div className="text-4xl font-black tracking-tighter text-slate-800">
        STARTUP<span className="text-cyan-600">SC</span>
        </div>

        <div ref={navRef} className="relative hidden md:flex items-center gap-8 font-semibold">
        {links.map((link, i) => (
            <NavLink
            key={link.to}
            to={link.to}
