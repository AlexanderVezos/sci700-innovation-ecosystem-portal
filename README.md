...
CORS_ORIGIN=         # your public URL once deployed, e.g. https://yourapp.onrender.com
...
## 3. Hosting with Render

Render deploys the portal to a public URL for free with no time limit. Free tier services sleep after 15 minutes of inactivity and take about 50 seconds to wake up on the first visit -- open the URL yourself before any showcase to avoid this.

1. Sign up at [render.com](https://render.com)
2. The repository must be set to **public** on GitHub before Render can see it -- you can make it private again after the initial deploy if needed
3. Click **New > Web Service** and select this repository
4. On the configuration page:
   - **Name** -- anything you like
   - **Language** -- Node (auto-detected, leave it)
   - **Branch** -- main (leave it)
   - **Region** -- leave as default
   - **Root Directory** -- leave blank
   - **Build Command** -- set to `npm install --legacy-peer-deps && npm run build`
   - **Start Command** -- leave as `npm run start`
   - **Instance Type** -- select **Free**
5. Scroll down to **Environment Variables** and add each value from your `.env` file -- **do this before clicking Deploy or the site will fail to start**
6. Click **Deploy** -- Render provides a public URL once the build completes
...
For persistent deployment, use your preferred process manager to run `npm run start`.
